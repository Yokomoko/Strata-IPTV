...
data class SeriesEntity(
    ...
    @ColumnInfo(name = "provider") val provider: String = "",
    /**
     * Snapshot of `total_episodes` at the moment the user last engaged
     * with this series (visited its detail screen, or before a sync
     * brought new episodes in).  The "NEW episodes" badge fires when
     * `total_episodes > last_seen_total_episodes`; the show detail
     * screen marks them seen on load.
     */
    ...
    val hasNewEpisodes: Boolean
        get() = totalEpisodes > 0 && totalEpisodes > lastSeenTotalEpisodes
}
