...
class MovieDeduplicator @Inject constructor(
     ...
     */
    /** Instance-level alias so existing callers don't need updating. */
    internal fun detectQuality(displayName: String, streamUrl: String = ""): Quality =
        Companion.detectQuality(displayName, streamUrl)
    ...
    companion object {
         ...
         * Detect the quality tier from a display name and/or stream URL.
         *
         * Exposed as a companion function so [SyncService] can use it for
         * episode deduplication during import without needing a full
         * [MovieDeduplicator] instance.
         */
        fun detectQuality(displayName: String, streamUrl: String = ""): Quality {
            ...
            if ("HEVC" in dn) return Quality.HevcFhd
